...
/**
 * Busca recursivamente o item do menu pelo path
 */
...
/**
 * Obtém informações do item do menu para favoritos
 */
export const getMenuItemInfo = (
  ...
  // Busca a categoria (primeiro nível pai)
  const findCategory = (items: MenuItemProps[], targetPath: string, parent?: MenuItemProps): string => {
    for (const menuItem of items) {
      ...
      if (menuItem.children) {
        const result = findCategory(menuItem.children, targetPath, parent || menuItem)
  ...
  const category = findCategory(items, path)

  // Mapeia ícone baseado na categoria
  ...
  return {
    title: item.label,
    category: category,
    icon: iconMap[category] || iconMap[item.label] || '📄'
  }
}