...
export default function EpiScanScreen({
  ...
  // ── Auto-start stream — polling até o vídeo estar pronto ───────────────
  const autoStartedRef = useRef(false);
  ...
  useEffect(() => {
    ...
    // Polling a cada 500ms até encontrar o elemento de vídeo pronto
    const interval = setInterval(() => {
      if (autoStartedRef.current) { clearInterval(interval); return; }
      if (stream.status !== 'idle') { clearInterval(interval); return; }

      // Tenta encontrar o vídeo pelo data-role ou pelo ref
      const video = videoRef.current
        ?? (document.querySelector('video[data-role="body1"]') as HTMLVideoElement | null);

      if (!video) return; // ainda não montou

      // Vídeo encontrado — inicia o stream
      clearInterval(interval);
      autoStartedRef.current = true;
      videoRef.current = video;
      console.log('[EpiScanScreen] Auto-iniciando stream WS — video pronto');
      stream.startStream(video);
    }, 500);

    return () => clearInterval(interval);
  }, [enableAutoCapture]); // roda só uma vez ao montar
          ...
          <div style={{ position: 'absolute', bottom: '1.5rem', left: '1rem', right: '1rem', display: 'flex', flexDirection: 'column', gap: '0.75rem', zIndex: 10 }}>
            ...
            {!isStreaming && !isDecided && (
              <button
                ...
                style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.75rem', padding: '1rem', fontSize: '0.95rem', background: 'rgba(0,132,255,0.9)', backdropFilter: 'blur(12px)', border: '2px solid rgba(0,132,255,0.4)' }}
              >
                <span style={{ fontSize: '1.25rem' }}>{stream.status === 'connecting' ? '⏳' : '🎥'}</span>
                <span>{stream.status === 'connecting' ? 'CONECTANDO...' : 'INICIAR VERIFICAÇÃO'}</span>
              </button>
            )}
            ...
            {enableAutoCapture && stream.status === 'idle' && (
              <div style={{ textAlign: 'center', fontFamily: 'var(--font-mono)', fontSize: '0.7rem', color: 'rgba(255,255,255,0.5)' }}>
                💡 Análise iniciará automaticamente
              </div>
            )}
          </div>
