...
const AssetFinancialGrid: React.FC<AssetDetailsTableProps> = ({ initialFilters }) => {
  ...
  const exportToPDF = async (data: any[], scope: string) => {
    ...
    doc.text('Asset Financial Report', 14, 15);
    
    doc.setFontSize(9);
    doc.setTextColor(107, 114, 128); // gray-600
    doc.text(`Export Date: ${new Date().toLocaleString()}`, 14, 22);
    doc.text(`Scope: ${scope === 'filtered' ? `Filtered Results (${data.length} assets)` : `All Assets (${data.length} assets)`}`, 14, 27);
    
    if (Object.keys(filters).length > 0) {
      doc.text(`Filters Applied: ${Object.keys(filters).length}`, 14, 32);
    }
    ...
    autoTable(doc, {
      head: [['Asset Code', 'Asset Name', 'Category', 'Department', 'Purchase Cost', 'Current Value', 'Insurance', 'Status']],
      body: tableData,
      startY: Object.keys(filters).length > 0 ? 37 : 32,
      theme: 'striped',
      headStyles: {
        fillColor: [91, 147, 255], // primary-500
      ...
      columnStyles: {
        0: { cellWidth: 25 }, // Asset Code
        1: { cellWidth: 55 }, // Asset Name
        2: { cellWidth: 35 }, // Category
        3: { cellWidth: 35 }, // Department
        4: { cellWidth: 28 }, // Purchase Cost
        5: { cellWidth: 28 }, // Current Value
        6: { cellWidth: 22 }, // Insurance
      ...
      didDrawCell: (data: any) => {
        ...
        if (data.column.index === 6 && data.cell.section === 'body') {
          ...
          if (value === 'Active') {
            ...
            doc.setTextColor(6, 95, 70); // green-900
    ...
    // Save PDF
    const fileName = `asset-financial-${scope}-${new Date().toISOString().split('T')[0]}.pdf`;
    doc.save(fileName);
  };
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gradient-to-r from-gray-50 to-gray-100 sticky top-0 z-10">
              <tr>
                ...
                <SortHeader field="current_book_value">Current Value</SortHeader>
                <SortHeader field="insurance_status">Insurance</SortHeader>
                <SortHeader field="active_status">Status</SortHeader>
                <th className="px-3 py-1.5 text-center text-[10px] font-bold text-gray-700 uppercase tracking-wide">
                  Actions
                </th>
              </tr>
            </thead>
