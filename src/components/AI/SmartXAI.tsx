...
function formatMessage(text: string) {
  ...
  return lines.map((line, i) => {
    if (line.startsWith('## ')) {
      return <h3 key={i} className="text-sm font-bold text-primary-700 dark:text-primary-300 mt-3 mb-1">{line.replace('## ', '')}</h3>
    }
    if (line.startsWith('### ')) {
      return <h4 key={i} className="text-xs font-semibold text-gray-700 dark:text-gray-300 mt-2 mb-1">{line.replace('### ', '')}</h4>
    }
    if (line.startsWith('**') && line.endsWith('**')) {
      return <p key={i} className="text-xs font-semibold text-gray-800 dark:text-gray-200">{line.replace(/\*\*/g, '')}</p>
    }
    if (line.match(/^\d+\.\s/)) {
      return <p key={i} className="text-xs text-gray-700 dark:text-gray-300 ml-2">• {line.replace(/^\d+\.\s/, '')}</p>
    }
    if (line.startsWith('- ') || line.startsWith('• ')) {
      return <p key={i} className="text-xs text-gray-700 dark:text-gray-300 ml-2">• {line.replace(/^[-•]\s/, '')}</p>
    }
    if (line.trim() === '') {
      return <br key={i} />
    }
    return <p key={i} className="text-xs text-gray-700 dark:text-gray-300 leading-relaxed">{line}</p>
  })
}
