...
//   };
  ...
  const getStatus = (score: string) => {
    const numericScore = getNumericValue(score);
    if (numericScore >= 80) return 'Excelente';
    if (numericScore >= 60) return 'Bom';
    if (numericScore >= 40) return 'Regular';
    return 'Crítico';
  };
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-64 p-8">
        ...
        <p className="mt-4 text-gray-600">Carregando dados de Health Score...</p>
      </div>
    );
  }
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div>
              ...
              <p className={`text-2xl font-bold ${
                parseFloat(realAverage) >= 60 ? 'text-green-600' : 'text-red-600'
              }`}>
                {realAverage}%
              </p>
            </div>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div className="flex items-center">
              <InformationCircleIcon className="h-6 w-6 text-gray-400 mr-3" />
              <div>
                <p className="text-sm font-medium text-gray-600">Melhor Categoria</p>
                <p className="text-lg font-bold text-green-600">
                  {summary.best_category?.VALUE || '0.0'}%
                </p>
                <p className="text-xs text-gray-500 truncate">
                  {summary.best_category?.NAME || 'N/A'}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="flex items-center space-x-4">
              <span className="text-sm text-gray-500">
                {data.length} categorias • {totalDevices} dispositivos
              </span>
              <select 
                value={sortOrder}
                onChange={(e) => {
        ...
        <div className="px-6 py-4 border-t border-gray-200">
          <div className="flex justify-between items-center">
            <p className="text-sm text-gray-700">
              ...
              <span className="font-medium">{sortedData.length}</span> categorias
            </p>
