...
// // Proxy Node.js — mesmo servidor que serve o front.
// // wss:// porque o Cloudflare/Nginx faz TLS por fora.
// // O servidor Node faz bridge local para ws://<ip>:81.
// // Ver: server.js → WebSocketServer path="/ws/lock"
// // ─────────────────────────────────────────────────────────────────────────────
...
//   /** Lista de eventos recebidos em tempo real */
//   events: ValidationStreamEvent[];
//   /** Último evento recebido */
//   latest: ValidationStreamEvent | null;
//   /** true = stream aberto e conectado */
//   streaming: boolean;
//   /** Abre o stream para uma sessão */
//   open: (sessionUuid: string) => void;
//   /** Fecha o stream manualmente */
//   close: () => void;
// }
...
// /**
//  * DoorStatus — reflete o estado REAL do ESP32
//  *
//  * closed    → lock_state=locked  + door_state=closed  (estado normal)
//  * open      → lock_state=unlocked                      (fechadura destravada)
//  * door_open → door_state=open    (porta fisicamente aberta, reed switch)
//  * alert     → falha de comunicação ou unlock negado
//  * waiting   → aguardando primeira resposta
//  */
// export type DoorStatus = "closed" | "open" | "door_open" | "alert" | "waiting";
...
// // ─────────────────────────────────────────────────────────────────────────────
// // HOOK: useValidationStream — SSE de resultados de validação em tempo real
// //
// // Conecta em GET /api/v1/epi/validation/{session_uuid}/stream
// // Recebe eventos: connected | photo_result | session_complete
// // ─────────────────────────────────────────────────────────────────────────────

// function useValidationStream(
//   companyId: number,
//   onEvent?: (event: ValidationStreamEvent) => void,
//   onComplete?: (event: ValidationStreamEvent) => void,
// ): ValidationStreamState {
//   const [events, setEvents] = useState<ValidationStreamEvent[]>([]);
//   const [latest, setLatest] = useState<ValidationStreamEvent | null>(null);
...
//   const open = useCallback((sessionUuid: string) => {
...
//     const url = `${EPI}/validation/${sessionUuid}/stream?company_id=${companyId}`;
//     const es = new EventSource(url);
//     esRef.current = es;

//     es.addEventListener("connected", () => {
//       setStreaming(true);
//       console.log(`[SSE] Stream aberto para sessão ${sessionUuid}`);
//     });

//     es.addEventListener("photo_result", (e: MessageEvent) => {
//       try {
//         const data: ValidationStreamEvent = JSON.parse(e.data);
//         setLatest(data);
//         setEvents((prev) => [...prev, data]);
//         onEventRef.current?.(data);
//         console.log(`[SSE] photo_result seq=${data.photo_seq} epi=${data.epi_compliant} face=${data.face_detected}`);
...
//         console.log(`[SSE] session_complete → ${data.final_decision?.access_decision}`);
//       } catch { /* ignora parse error */ }
//       setStreaming(false);
//       es.close();
//       esRef.current = null;
...
//     es.onerror = () => {
//       console.warn("[SSE] Erro no stream — fechando");
//       setStreaming(false);
//       es.close();
//       esRef.current = null;
//     };
...
//   return { events, latest, streaming, open, close };
// }
...
//   // ─── SSE de validação ─────────────────────────────────────────────────────
//   const validationStream = useValidationStream(
//     sysConfig.companyId,
//     (evt) => console.log("[SSE live]", evt.photo_seq, evt.epi_compliant),
//   );

//   // ─────────────────────────────────────────────────────────────────────────
//   // POLLING REST — GET http://<ip>/status a cada 3s
//   //
//   // Fallback quando o WebSocket não está disponível (Mixed Content ou rede).
//   // Para automaticamente quando lockState.connected = true.
//   // Útil para testar status da porta sem WebSocket funcional.
//   // ─────────────────────────────────────────────────────────────────────────
//   useEffect(() => {
//     const lockIp = sysConfig.lockIpAddress;

//     // Se WebSocket está conectado, não precisa de polling
//     if (lockState.connected) return;

//     const poll = async () => {
//       try {
//         const res = await fetch(`http://${lockIp}/status`, {
//           signal: AbortSignal.timeout(2000),
//         });
//         if (!res.ok) return;
//         const data = await res.json();
//         console.log("[DoorPoll]", data);

//         if (data.lock_state === "unlocked") {
//           setDoorStatus("open");
...
//         }
//       } catch {
//         // silencia — ESP32 pode estar inacessível ou Mixed Content bloqueou
//       }
//     };
...
//   permanenceState: PermanenceState;
//   lockState: LockState;
//   /** Stream SSE de resultados de validação em tempo real */
...
//   // ─────────────────────────────────────────────────────────────────────────
//   // POLLING REST — GET http://<ip>/status a cada 30s
...
//         }
//       } catch {
//         // silencia — ESP32 pode estar inacessível
//       }
//     };
...
//     setDoorStatus("closed"); setScreen("idle");
//   }, [cameraHook, resetSession, resetFaceScan, resetEpiScan, validationStream]);
...
//       // ── Abre SSE para feedback em tempo real ──────────────────────────────
//       validationStream.open(epiSessionUuid);
//       // ─────────────────────────────────────────────────────────────────────
...
//     }
//     // eslint-disable-next-line react-hooks/exhaustive-deps
//   }, [epiStep, cameraHook, direction, sysConfig, lockState.connected, lockState.sendLed, validationStream]);
...
//     screen, direction, doorStatus, session, sysConfig,
//     showReport, showConfig, setShowReport, setShowConfig,
//     cameraHook, lockState, validationStream,
...
// const makeHttp = () => axios.create({ baseURL: getApiBaseUrl(), timeout: 60000 });
// const EPI = "https://aihub.smartxhub.cloud/api/v1/epi";
...
// export interface PeopleFilters { activeOnly?: boolean; isInside?: boolean; hasPhotos?: boolean; search?: string; limit?: number; offset?: number; }
...
// export interface ValidationStreamEvent {
//   session_uuid: string; photo_seq: number; photo_count_received: number; photo_count_required: number;
//   session_complete: boolean; face_detected: boolean; face_confidence: number | null; face_person_code: string | null;
//   face_bbox?: { x: number; y: number; w: number; h: number } | null;
//   epi_compliant: boolean; compliance_score: number; missing: string[];
//   detections?: EpiDetection[];
//   processing_ms: number;
//   final_decision?: { access_decision: string; epi_compliant: boolean; face_confirmed: boolean; person_code?: string; person_name?: string; } | null;
// }

// export interface ValidationStreamState {
//   events: ValidationStreamEvent[]; latest: ValidationStreamEvent | null; streaming: boolean;
//   open: (sessionUuid: string) => void; close: () => void;
// }

// // ─── Tipo da decisão do WS EPI Stream ────────────────────────────────────────
// export interface EpiStreamDecision {
//   access_decision: "GRANTED" | "DENIED_EPI" | "DENIED_FACE";
//   compliant_frames: number; total_frames: number; compliance_rate: number;
//   face_frames: number; face_rate: number;
//   person_code: string | null; person_name: string | null;
// }

// const api = {
//   getLocalConfig: async (): Promise<Partial<SysConfig>> => { try { const { data } = await makeHttp().get(`${EPI}/local/config`); return data; } catch { return {}; } },
//   getDashboard: async (): Promise<DashboardData> => { const { data } = await makeHttp().get(`${EPI}/analytics/dashboard`); return data; },
//   getPeople: async (params: PeopleFilters = {}): Promise<WorkerRecord[]> => {
//     const { data } = await makeHttp().get(`${EPI}/analytics/people`, { params: { active_only: params.activeOnly ?? false, is_inside: params.isInside, has_photos: params.hasPhotos, search: params.search, limit: params.limit ?? 500, offset: params.offset ?? 0 } });
//     return Array.isArray(data) ? data : ((data as { people?: WorkerRecord[] }).people ?? []);
//   },
//   getEpiConfig: async (): Promise<EpiConfig> => { const { data } = await makeHttp().get(`${EPI}/config`); return { required_ppe: data.config?.required_ppe ?? [], available_classes: data.all_classes ? (Object.values(data.all_classes) as string[]) : [], config: data.config }; },
//   saveEpiConfig: async (config: { required_ppe: string[] }): Promise<void> => { const ALL_PPE = ["thermal_coat", "thermal_pants", "gloves", "helmet", "boots", "person"]; const body = Object.fromEntries(ALL_PPE.map((cls) => [cls, config.required_ppe.includes(cls)])); body["person"] = true; await makeHttp().post(`${EPI}/config`, body); },
//   startValidationSession: async (overrides: Record<string, unknown> = {}): Promise<{ session_uuid: string; sessionUuid?: string }> => { const form = new FormData(); form.append("door_id", String(overrides.door_id ?? "DOOR_01")); form.append("direction", String(overrides.direction ?? "ENTRY")); form.append("zone_id", String(overrides.zone_id ?? "")); form.append("compliance_mode", "majority"); form.append("photo_count_required", "1"); form.append("timeout_seconds", "30"); const { data } = await makeHttp().post(`${EPI}/validation/start`, form); return data; },
//   sendValidationPhoto: async (sessionUuid: string, frameBlob: Blob, opts: { photoType?: string; cameraId?: number } = {}): Promise<PhotoResult> => { const form = new FormData(); form.append("session_uuid", sessionUuid); form.append("file", frameBlob, "frame.jpg"); if (opts.cameraId !== undefined) form.append("camera_id", String(opts.cameraId)); if (opts.photoType) form.append("photo_type", opts.photoType); const { data } = await makeHttp().post(`${EPI}/validation/photo`, form, { headers: { "Content-Type": "multipart/form-data" } }); return data; },
//   closeValidationSession: async (sessionUuid: string, personCode?: string): Promise<void> => { const form = new FormData(); form.append("session_uuid", sessionUuid); if (personCode) form.append("person_code_override", personCode); await makeHttp().post(`${EPI}/validation/close`, form); },
//   openDoor: async (payload: { personCode?: string; personName?: string; sessionUuid?: string | null; reason: string }): Promise<void> => { try { const form = new FormData(); if (payload.personCode) form.append("person_code", payload.personCode); if (payload.personName) form.append("person_name", payload.personName); if (payload.sessionUuid) form.append("session_uuid", payload.sessionUuid); form.append("reason", payload.reason); await makeHttp().post(`${EPI}/door/open`, form); } catch (e) { console.warn("[openDoor] endpoint não disponível:", e); } },
//   unlockDoor: async (lockIp: string, durationMs = 5000): Promise<void> => { const res = await fetch(`http://${lockIp}/unlock`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ duration_ms: durationMs }) }); if (!res.ok) throw new Error(`ESP32 HTTP ${res.status}`); const data = await res.json(); if (!data.ok) throw new Error("ESP32 recusou o unlock"); },
// };

// function canvasToBlob(canvas: HTMLCanvasElement, quality = 0.88): Promise<Blob> { return new Promise((resolve) => canvas.toBlob((b) => resolve(b!), "image/jpeg", quality)); }

// export function formatMinutes(mins: number | null | undefined): string { if (mins == null) return "—"; const m = Math.round(mins); if (m < 60) return `${m}min`; const h = Math.floor(m / 60); const rem = m % 60; return rem > 0 ? `${h}h ${rem}min` : `${h}h`; }

// export type Screen = "idle" | "face_scan" | "time_alert" | "epi_scan" | "access_granted" | "access_denied";
// export type Direction = "ENTRY" | "EXIT";
// export type DoorStatus = "closed" | "open" | "door_open" | "alert" | "waiting";
// export type CamRole = "face" | "body1" | "body2";
// export type CameraSourceType = "local" | "ip_url";
// export interface CameraSource { type: CameraSourceType; deviceId?: string; url?: string; }
// export interface Person { personCode: string; personName: string; confidence: number; }
// export interface DailyExposure { total_minutes: number; limit_minutes: number; entries_today?: number; }
// export interface EpiResult { compliant: boolean; detected?: string[]; detected_ppe?: string[]; missing?: string[]; missing_ppe?: string[]; }
// export interface Session { sessionUuid: string | null; person: Person | null; dailyExposure: DailyExposure | null; epiResult: EpiResult | null; missingEpi: string[]; deniedReason?: "user_not_found" | "epi_incomplete" | null; }
// export interface SysConfig { companyId: number; zoneId: number; doorId: string; dailyLimitMin: number; overLimitPolicy: "warn" | "block"; doorOpenMaxMin: number; faceConfidenceMin: number; apiBase: string; useSingleCamera: boolean; cameraSourceType: Record<CamRole, CameraSourceType>; cameraIpUrl: Record<CamRole, string | null>; lockIpAddress: string; lockDurationMs: number; }
// export interface EpiConfig { required_ppe: string[]; available_classes?: string[]; config?: Record<string, unknown>; }
// export interface DashboardData { inside_count?: number; people_inside?: number; entries_today?: number; today?: { total?: number }; open_alerts?: number; alerts_open?: number; over_limit_count?: number; }
// export interface WorkerRecord { person_code: string; person_name: string; department?: string; badge_id?: string; is_inside?: boolean; face_photos_count?: number; last_entry_at?: string | null; last_exit_at?: string | null; active?: boolean; created_at?: string; daily_accumulated_min?: number; total_minutes?: number; total_entries?: number; sessions_today?: number; }
// export interface CamDevice { deviceId: string; label: string; kind: string; }
// export interface CameraHook { devices: CamDevice[]; assignments: Record<CamRole, string | null>; streams: Partial<Record<CamRole, MediaStream>>; loading: boolean; error: string | null; enumerateDevices: () => Promise<CamDevice[]>; startStream: (role: CamRole) => Promise<MediaStream | null>; stopStream: (role: CamRole) => void; stopAllStreams: () => void; captureFrame: (role: CamRole) => Promise<{ blob: Blob; dataUrl: string; canvas: HTMLCanvasElement }>; assignDevice: (role: CamRole, deviceId: string | null) => void; setVideoRef: (role: CamRole, element: HTMLVideoElement | null) => void; hasStream: (role: CamRole) => boolean; getAssignment: (role: CamRole) => string | null; sourceTypes: Record<CamRole, CameraSourceType>; ipUrls: Record<CamRole, string | null>; setSourceType: (role: CamRole, type: CameraSourceType) => void; setIpUrl: (role: CamRole, url: string | null) => void; }
// export type LockWsEvent = "connected" | "status" | "locked" | "unlocked" | "door_open" | "door_closed" | "button_press" | "proxy_connected" | "proxy_error" | "proxy_esp_disconnected";
// export interface LockWsMessage { event: LockWsEvent; lock_state?: "locked" | "unlocked"; door_state?: "open" | "closed"; led?: string; duration_ms?: number; event_count?: number; btn_count?: number; uptime_ms?: number; ip?: string; version?: string; ws_clients?: number; esp_url?: string; lock_ip?: string; error?: string; detail?: string; reason?: string; }
// export interface LockState { connected: boolean; lockState: "locked" | "unlocked" | null; doorState: "open" | "closed" | null; led: string | null; version: string | null; eventCount: number; lastEvent: LockWsEvent | null; lastEventAt: Date | null; error: string | null; wsUrl: string | null; sendUnlock: (durationMs?: number) => void; sendLock: () => void; sendLed: (color: "red" | "green" | "blue" | "yellow" | "off") => void; }
// interface PhotoResult { session_uuid?: string; face_detected?: boolean; face_recognized?: boolean; face_confidence?: number; face_person_code?: string; person_code?: string; person_name?: string; confidence?: number; epi_compliant?: boolean; compliant?: boolean; compliance_score?: number; missing?: string[]; missing_ppe?: string[]; detected?: string[]; detected_ppe?: string[]; final_decision?: { access_decision: string; epi_compliant: boolean; face_confirmed: boolean; face_confidence_max?: number; person_code?: string; person_name?: string; } | null; daily_exposure?: DailyExposure; }
// export type FaceScanStep = "ready" | "capturing" | "processing" | "done" | "error";
// export interface FaceScanState { step: FaceScanStep; captureUrl: string | null; progress: number; statusMsg: string; subMsg: string; countdown: number | null; }
// export type EpiScanStep = "ready" | "capturing" | "processing" | "done" | "error";
// export interface EpiScanState { step: EpiScanStep; captureUrl1: string | null; captureUrl2: string | null; statusMsg: string; countdown: number | null; result: EpiResult | null; }
// export interface IdleState { dashboard: DashboardData | null; loadingDash: boolean; refreshDashboard: () => void; }
// export interface ConfigState { localConfig: SysConfig; epiConfig: EpiConfig | null; saving: boolean; saved: boolean; setLocalConfig: (cfg: SysConfig) => void; setEpiConfig: (cfg: EpiConfig | null) => void; handleSave: () => Promise<void>; }
// export interface PermanenceState { people: WorkerRecord[]; loading: boolean; fetchPeople: (filters?: PeopleFilters) => Promise<void>; }

// export interface UseCamAutomationReturn {
//   screen: Screen; direction: Direction; doorStatus: DoorStatus; session: Session; sysConfig: SysConfig;
//   showReport: boolean; showConfig: boolean; setShowReport: (v: boolean) => void; setShowConfig: (v: boolean) => void;
//   cameraHook: CameraHook; idleState: IdleState; faceScanState: FaceScanState; epiScanState: EpiScanState;
//   configState: ConfigState; permanenceState: PermanenceState; lockState: LockState; validationStream: ValidationStreamState;
//   handleStartEntry: () => void; handleStartExit: () => void; handleGoIdle: () => void;
//   handleTimeOverride: () => void; handleRetryFromDenied: () => void;
//   handleSaveConfig: (newConfig: Partial<SysConfig>) => void;
//   handleCaptureFace: () => Promise<void>; handleRetryFace: () => void;
//   handleCaptureEpi: () => Promise<void>; handleRetryEpi: () => void;
//   /** Processa decisão do WS EPI Stream e navega para tela correta — sem chamadas de rede */
//   handleEpiStreamDecision: (decision: EpiStreamDecision) => void;
// }

// const LS_KEYS: Record<CamRole, string> = { face: "epi_cam_face", body1: "epi_cam_body1", body2: "epi_cam_body2" };
// const LS_KEYS_TYPE: Record<CamRole, string> = { face: "epi_cam_face_type", body1: "epi_cam_body1_type", body2: "epi_cam_body2_type" };
// const LS_KEYS_URL: Record<CamRole, string> = { face: "epi_cam_face_url", body1: "epi_cam_body1_url", body2: "epi_cam_body2_url" };
// const EMPTY_SESSION: Session = { sessionUuid: null, person: null, dailyExposure: null, epiResult: null, missingEpi: [], deniedReason: null };
// const DEFAULT_CONFIG: SysConfig = { companyId: 1, zoneId: 10, doorId: "DOOR_CAMARA_FRIA_01", dailyLimitMin: 120, overLimitPolicy: "warn", doorOpenMaxMin: 15, faceConfidenceMin: 70, apiBase: "https://aihub.smartxhub.cloud", useSingleCamera: false, cameraSourceType: { face: "local", body1: "local", body2: "local" }, cameraIpUrl: { face: null, body1: null, body2: null }, lockIpAddress: "192.168.68.100", lockDurationMs: 5000 };
// const EPI_LABELS_PT: Record<string, string> = { helmet: "Capacete", vest: "Colete", gloves: "Luvas", boots: "Botas", thermal_coat: "Jaqueta Térmica", thermal_pants: "Calça Térmica", glasses: "Óculos de Proteção", mask: "Máscara", apron: "Avental", hardhat: "Capacete" };
// const epiLabel = (k: string) => EPI_LABELS_PT[k] ?? k;

// async function captureFrameFromUrl(url: string): Promise<Blob> { const proxyUrl = `${getApiBaseUrl()}/api/v1/epi/camera/snapshot/v4?url=${encodeURIComponent(url)}`; const response = await fetch(proxyUrl); if (!response.ok) throw new Error(`HTTP ${response.status}`); return response.blob(); }
// function resolveCompliant(photo: PhotoResult): boolean { return photo.epi_compliant ?? photo.final_decision?.epi_compliant ?? photo.compliant ?? false; }

// // ─────────────────────────────────────────────────────────────────────────────
// // HOOK: useLockWebSocket
// // ─────────────────────────────────────────────────────────────────────────────
// function useLockWebSocket(lockIp: string, onDoorStatusChange?: (status: DoorStatus) => void): LockState {
//   const [connected, setConnected] = useState(false);
//   const [lockState, setLockState] = useState<"locked" | "unlocked" | null>(null);
//   const [doorState, setDoorState] = useState<"open" | "closed" | null>(null);
//   const [led, setLed] = useState<string | null>(null);
...
//   const reconnectTimer = useRef<ReturnType<typeof setTimeout> | null>(null);
//   const reconnectDelay = useRef(2000);
//   const onStatusRef = useRef(onDoorStatusChange); onStatusRef.current = onDoorStatusChange;
//   const lockStateRef = useRef(lockState); lockStateRef.current = lockState;
//   const doorStateRef = useRef(doorState); doorStateRef.current = doorState;

//   const connect = useCallback(() => {
//     if (!lockIp) return;
//     const url = getLockWsUrl(lockIp); setWsUrl(url);
//     const ws = new WebSocket(url); wsRef.current = ws;
//     ws.onopen = () => { setError(null); reconnectDelay.current = 2000; };
//     ws.onmessage = (ev) => {
//       let msg: LockWsMessage; try { msg = JSON.parse(ev.data); } catch { return; }
//       setLastEvent(msg.event); setLastEventAt(new Date());
//       if (msg.event === "proxy_connected") { setConnected(true); setError(null); return; }
//       if (msg.event === "proxy_error") { setError(msg.error ?? "Erro"); setConnected(false); onStatusRef.current?.("alert"); return; }
//       if (msg.event === "proxy_esp_disconnected") { setConnected(false); onStatusRef.current?.("alert"); return; }
//       if (msg.event === "connected" || msg.event === "status") { setConnected(true); if (msg.lock_state) setLockState(msg.lock_state); if (msg.door_state) setDoorState(msg.door_state); if (msg.led) setLed(msg.led); if (msg.version) setVersion(msg.version); if (msg.event_count !== undefined) setEventCount(msg.event_count); if (msg.lock_state === "unlocked") onStatusRef.current?.("open"); else if (msg.door_state === "open") onStatusRef.current?.("door_open"); else onStatusRef.current?.("closed"); return; }
//       if (msg.event === "unlocked") { setLockState("unlocked"); if (msg.event_count !== undefined) setEventCount(msg.event_count); onStatusRef.current?.("open"); return; }
//       if (msg.event === "locked") { setLockState("locked"); if (doorStateRef.current !== "open") onStatusRef.current?.("closed"); return; }
//       if (msg.event === "door_open") { setDoorState("open"); onStatusRef.current?.("door_open"); return; }
//       if (msg.event === "door_closed") { setDoorState("closed"); if (lockStateRef.current !== "unlocked") onStatusRef.current?.("closed"); return; }
//     };
//     ws.onerror = () => { setError("Erro de conexão com a fechadura"); onStatusRef.current?.("alert"); };
//     ws.onclose = () => { setConnected(false); wsRef.current = null; reconnectTimer.current = setTimeout(() => { reconnectDelay.current = Math.min(reconnectDelay.current * 1.5, 30000); connect(); }, reconnectDelay.current); };
//   }, [lockIp]); // eslint-disable-line react-hooks/exhaustive-deps
...
//     setConnected(false); setLockState(null); setDoorState(null); setError(null); reconnectDelay.current = 2000;
//     connect();
//     return () => { if (reconnectTimer.current) clearTimeout(reconnectTimer.current); if (wsRef.current) { wsRef.current.onclose = null; wsRef.current.close(); wsRef.current = null; } };
//   }, [lockIp, connect]);

//   const send = useCallback((p: object) => { if (wsRef.current?.readyState === WebSocket.OPEN) wsRef.current.send(JSON.stringify(p)); }, []);
//   const sendUnlock = useCallback((ms?: number) => send({ action: "unlock", duration_ms: ms ?? DEFAULT_CONFIG.lockDurationMs }), [send]);
//   const sendLock = useCallback(() => send({ action: "lock" }), [send]);
//   const sendLed = useCallback((color: "red" | "green" | "blue" | "yellow" | "off") => send({ action: "led", color }), [send]);
//   return { connected, lockState, doorState, led, version, eventCount, lastEvent, lastEventAt, error, wsUrl, sendUnlock, sendLock, sendLed };
// }

// // ─────────────────────────────────────────────────────────────────────────────
// // HOOK: useValidationStream
// // ─────────────────────────────────────────────────────────────────────────────
// function useValidationStream(companyId: number, onEvent?: (e: ValidationStreamEvent) => void, onComplete?: (e: ValidationStreamEvent) => void): ValidationStreamState {
//   const [events, setEvents] = useState<ValidationStreamEvent[]>([]);
//   const [latest, setLatest] = useState<ValidationStreamEvent | null>(null);
//   const [streaming, setStreaming] = useState(false);
//   const esRef = useRef<EventSource | null>(null);
...
//   const close = useCallback(() => { if (esRef.current) { esRef.current.close(); esRef.current = null; } setStreaming(false); }, []);
//   const open = useCallback((sessionUuid: string) => {
//     if (esRef.current) { esRef.current.close(); esRef.current = null; }
//     setEvents([]); setLatest(null);
//     const es = new EventSource(`${EPI}/validation/${sessionUuid}/stream?company_id=${companyId}`);
//     esRef.current = es;
//     es.addEventListener("connected", () => setStreaming(true));
//     es.addEventListener("photo_result", (e: MessageEvent) => { try { const d: ValidationStreamEvent = JSON.parse(e.data); setLatest(d); setEvents((p) => [...p, d]); onEventRef.current?.(d); } catch {} });
//     es.addEventListener("session_complete", (e: MessageEvent) => { try { const d: ValidationStreamEvent = JSON.parse(e.data); setLatest(d); setEvents((p) => [...p, d]); onCompleteRef.current?.(d); } catch {} setStreaming(false); es.close(); esRef.current = null; });
...
// function useCameraInternal(sysConfig: SysConfig): CameraHook {
//   const [devices, setDevices] = useState<CamDevice[]>([]);
//   const [assignments, setAssignments] = useState<Record<CamRole, string | null>>({ face: localStorage.getItem(LS_KEYS.face) || null, body1: localStorage.getItem(LS_KEYS.body1) || null, body2: localStorage.getItem(LS_KEYS.body2) || null });
//   const [sourceTypes, setSourceTypes] = useState<Record<CamRole, CameraSourceType>>({ face: (localStorage.getItem(LS_KEYS_TYPE.face) as CameraSourceType) || "local", body1: (localStorage.getItem(LS_KEYS_TYPE.body1) as CameraSourceType) || "local", body2: (localStorage.getItem(LS_KEYS_TYPE.body2) as CameraSourceType) || "local" });
//   const [ipUrls, setIpUrls] = useState<Record<CamRole, string | null>>({ face: localStorage.getItem(LS_KEYS_URL.face) || null, body1: localStorage.getItem(LS_KEYS_URL.body1) || null, body2: localStorage.getItem(LS_KEYS_URL.body2) || null });
//   const [streams, setStreams] = useState<Partial<Record<CamRole, MediaStream>>>({});
//   const [loading, setLoading] = useState(false);
//   const [error, setError] = useState<string | null>(null);
//   const videoRefs = useRef<Partial<Record<CamRole, HTMLVideoElement | null>>>({});
//   const enumerateDevices = useCallback(async (): Promise<CamDevice[]> => {
//     try { await navigator.mediaDevices.getUserMedia({ video: true, audio: false }).then((s) => s.getTracks().forEach((t) => t.stop())).catch(() => {}); const all = await navigator.mediaDevices.enumerateDevices(); const vids = all.filter((d) => d.kind === "videoinput") as CamDevice[]; setDevices(vids); setAssignments((prev) => { const next = { ...prev }; if (!next.face && vids[0]) next.face = vids[0].deviceId; if (!next.body1 && vids[1]) next.body1 = vids[1].deviceId; if (!next.body2 && vids[2]) next.body2 = vids[2].deviceId; return next; }); return vids; }
...
//     }
//   }, [assignments, sourceTypes, streams]);
//   const stopStream = useCallback((role: CamRole) => { streams[role]?.getTracks().forEach((t) => t.stop()); setStreams((prev) => { const n = { ...prev }; delete n[role]; return n; }); const ref = videoRefs.current[role]; if (ref) ref.srcObject = null; }, [streams]);
//   const stopAllStreams = useCallback(() => { Object.values(streams).forEach((s) => s?.getTracks().forEach((t) => t.stop())); setStreams({}); Object.values(videoRefs.current).forEach((v) => { if (v) v.srcObject = null; }); }, [streams]);
//   const captureFrame = useCallback(async (role: CamRole) => {
//     if (sourceTypes[role] === "ip_url") { const url = ipUrls[role]; if (!url) throw new Error(`URL não configurada para câmera ${role}`); const blob = await captureFrameFromUrl(url); const dataUrl = URL.createObjectURL(blob); const canvas = document.createElement("canvas"); return { blob, dataUrl, canvas }; }
//     const video = videoRefs.current[role]; if (!video || video.readyState < 2) throw new Error(`Câmera "${role}" não está pronta.`);
...
//     const blob = await canvasToBlob(canvas, 0.88); const dataUrl = canvas.toDataURL("image/jpeg", 0.88); return { blob, dataUrl, canvas };
//   }, [sourceTypes, ipUrls]);
//   const assignDevice = useCallback((role: CamRole, deviceId: string | null) => { setAssignments((prev) => ({ ...prev, [role]: deviceId })); localStorage.setItem(LS_KEYS[role], deviceId || ""); }, []);
//   const setVideoRef = useCallback((role: CamRole, element: HTMLVideoElement | null) => { videoRefs.current[role] = element; if (element && streams[role]) element.srcObject = streams[role]!; }, [streams]);
//   const setSourceType = useCallback((role: CamRole, type: CameraSourceType) => { setSourceTypes((prev) => ({ ...prev, [role]: type })); localStorage.setItem(LS_KEYS_TYPE[role], type); }, []);
//   const setIpUrl = useCallback((role: CamRole, url: string | null) => { setIpUrls((prev) => ({ ...prev, [role]: url })); if (url) localStorage.setItem(LS_KEYS_URL[role], url); else localStorage.removeItem(LS_KEYS_URL[role]); }, []);
//   useEffect(() => { enumerateDevices(); const handler = () => enumerateDevices(); navigator.mediaDevices.addEventListener("devicechange", handler); return () => { navigator.mediaDevices.removeEventListener("devicechange", handler); stopAllStreams(); }; // eslint-disable-next-line react-hooks/exhaustive-deps
//   }, []);
//   return { devices, assignments, streams, loading, error, enumerateDevices, startStream, stopStream, stopAllStreams, captureFrame, assignDevice, setVideoRef, hasStream: (role) => !!streams[role], getAssignment: (role) => assignments[role], sourceTypes, ipUrls, setSourceType, setIpUrl };
// }
...
//   const handleLockStatusChange = useCallback((status: DoorStatus) => setDoorStatus(status), []);
//   const lockState = useLockWebSocket(sysConfig.lockIpAddress, handleLockStatusChange);
//   const validationStream = useValidationStream(sysConfig.companyId, (evt) => console.log("[SSE live]", evt.photo_seq, evt.epi_compliant));

//   useEffect(() => {
//     const lockIp = sysConfig.lockIpAddress; if (lockState.connected) return;
//     const poll = async () => { try { const res = await fetch(`http://${lockIp}/status`, { signal: AbortSignal.timeout(2000) }); if (!res.ok) return; const data = await res.json(); if (data.lock_state === "unlocked") setDoorStatus("open"); else if (data.door_state === "open") setDoorStatus("door_open"); else setDoorStatus("closed"); } catch {} };
//     poll(); const t = setInterval(poll, 30000); return () => clearInterval(t);
//   }, [sysConfig.lockIpAddress, lockState.connected]);

//   useEffect(() => { sessionRef.current = session; }, [session]);
//   const resetSession = useCallback(() => setSession(EMPTY_SESSION), []);
//   const resetFaceScan = useCallback((withAutoCapture = false) => { faceAutoCapture.current = withAutoCapture; setFaceStep("ready"); setFaceCaptureUrl(null); setFaceProgress(0); setFaceStatusMsg("Posicione seu rosto na câmera e toque em Capturar"); setFaceSubMsg(""); setFaceCountdown(null); }, []);
//   const resetEpiScan = useCallback((withAutoCapture = false) => { epiAutoCapture.current = withAutoCapture; setEpiStep("ready"); setEpiCaptureUrl1(null); setEpiCaptureUrl2(null); setEpiStatusMsg("Posicione-se em frente às câmeras de corpo"); setEpiCountdown(null); setEpiResult(null); }, []);

//   useEffect(() => { api.getLocalConfig().then((cfg) => { setSysConfig((prev) => ({ ...prev, ...cfg })); setLocalConfig((prev) => ({ ...prev, ...cfg })); }).catch((e: Error) => console.warn("[useCamAutomation] Config load failed:", e.message)); }, []);
//   const refreshDashboard = useCallback(async () => { try { const data = await api.getDashboard(); setDashboard(data); } catch (e) { console.warn("[useCamAutomation] Dashboard failed:", e); } finally { setLoadingDash(false); } }, []);
//   useEffect(() => { if (screen !== "idle") return; refreshDashboard(); const t = setInterval(refreshDashboard, 30000); return () => clearInterval(t); }, [screen, refreshDashboard]);
//   useEffect(() => { if (!showConfig) return; setLocalConfig({ ...sysConfig }); api.getEpiConfig().then(setEpiConfig).catch(console.warn); }, [showConfig, sysConfig]);
//   const fetchPeople = useCallback(async (filters: PeopleFilters = {}) => { try { setLoadingPeople(true); const list = await api.getPeople({ activeOnly: false, ...filters }); setPeople(list); } catch (e) { console.error("[useCamAutomation] fetchPeople failed:", e); } finally { setLoadingPeople(false); } }, []);
//   useEffect(() => { if (showReport) fetchPeople(); }, [showReport, fetchPeople]);

//   useEffect(() => {
//     if (screen !== "access_granted") { unlockFiredRef.current = false; return; }
//     if (unlockFiredRef.current) return; unlockFiredRef.current = true;
//     const lockMs = sysConfig.lockDurationMs; const lockIp = sysConfig.lockIpAddress;
//     if (lockState.connected) { lockState.sendUnlock(lockMs); lockState.sendLed("green"); }
//     else { api.unlockDoor(lockIp, lockMs).then(() => setDoorStatus("open")).catch(() => setDoorStatus("alert")); }
//     api.openDoor({ personCode: sessionRef.current.person?.personCode, personName: sessionRef.current.person?.personName, sessionUuid: sessionRef.current.sessionUuid, reason: "EPI_COMPLIANT" }).catch(console.warn);
//   }, [screen, sysConfig.lockIpAddress, sysConfig.lockDurationMs, lockState.connected]); // eslint-disable-line react-hooks/exhaustive-deps

//   useEffect(() => {
//     if (screen !== "face_scan" || !faceAutoCapture.current || faceStep !== "ready") return;
//     let n = 3; setFaceCountdown(n);
//     const t = setInterval(() => { n -= 1; if (n <= 0) { clearInterval(t); setFaceCountdown(null); handleCaptureFace(); } else setFaceCountdown(n); }, 1000);
//     return () => { clearInterval(t); setFaceCountdown(null); };
//     // eslint-disable-next-line react-hooks/exhaustive-deps
//   }, [screen, faceStep]);
...
//   useEffect(() => {
//     if (screen !== "epi_scan" || !epiAutoCapture.current || epiStep !== "ready") return;
//     let n = 4; setEpiCountdown(n);
//     const t = setInterval(() => { n -= 1; if (n <= 0) { clearInterval(t); setEpiCountdown(null); handleCaptureEpi(); } else setEpiCountdown(n); }, 1000);
//     return () => { clearInterval(t); setEpiCountdown(null); };
//     // eslint-disable-next-line react-hooks/exhaustive-deps
//   }, [screen, epiStep]);

//   const handleStartEntry = useCallback(() => { setDirection("ENTRY"); resetSession(); resetFaceScan(true); cameraHook.startStream("face"); setScreen("face_scan"); }, [resetSession, resetFaceScan, cameraHook]);
//   const handleStartExit = useCallback(() => { setDirection("EXIT"); resetSession(); resetFaceScan(true); cameraHook.startStream("face"); setScreen("face_scan"); }, [resetSession, resetFaceScan, cameraHook]);
//   const handleGoIdle = useCallback(() => { cameraHook.stopAllStreams(); resetSession(); resetFaceScan(false); resetEpiScan(false); validationStream.close(); setDoorStatus("closed"); setScreen("idle"); }, [cameraHook, resetSession, resetFaceScan, resetEpiScan, validationStream]);
//   const handleTimeOverride = useCallback(() => { resetEpiScan(true); if (!sysConfig.useSingleCamera) { cameraHook.startStream("body1"); if (cameraHook.getAssignment("body2")) cameraHook.startStream("body2"); } setScreen("epi_scan"); }, [cameraHook, resetEpiScan, sysConfig.useSingleCamera]);
//   const handleRetryFromDenied = useCallback(() => { setSession((prev) => ({ ...prev, epiResult: null, missingEpi: [], deniedReason: null })); resetEpiScan(false); setScreen("epi_scan"); }, [resetEpiScan]);
//   const handleSaveConfig = useCallback((newConfig: Partial<SysConfig>) => { setSysConfig((prev) => ({ ...prev, ...newConfig })); }, []);

//   const handleCaptureFace = useCallback(async () => {
//     if (faceStep !== "ready") return; faceAutoCapture.current = false;
//     try {
//       setFaceStep("capturing"); setFaceStatusMsg("Capturando frame…");
//       const { blob, dataUrl } = await cameraHook.captureFrame("face"); setFaceCaptureUrl(dataUrl);
//       setFaceStep("processing"); setFaceStatusMsg("Iniciando sessão de validação…"); setFaceProgress(20);
//       const sessionData = await api.startValidationSession({ direction, door_id: sysConfig.doorId, zone_id: sysConfig.zoneId });
//       const uuid = sessionData.session_uuid || sessionData.sessionUuid!; setFaceProgress(40);
//       setFaceStatusMsg("Reconhecendo rosto…");
//       const photo = await api.sendValidationPhoto(uuid, blob, { photoType: "face", cameraId: 1 }); setFaceProgress(80);
//       const resolvedCode = (photo.face_person_code || photo.person_code || photo.final_decision?.person_code || "").trim();
//       const resolvedName = (photo.person_name || photo.final_decision?.person_name || "").trim();
//       if (resolvedCode && resolvedName) {
//         setFaceProgress(100); setFaceStep("done");
//         const resolvedConf = photo.face_confidence || photo.confidence || photo.final_decision?.face_confidence_max || 0;
//         setFaceStatusMsg(`Identificado: ${resolvedName}`); setFaceSubMsg(`Confiança: ${Math.round(resolvedConf * 100)}%`);
//         const person: Person = { personCode: resolvedCode, personName: resolvedName, confidence: resolvedConf };
//         setSession((prev) => ({ ...prev, sessionUuid: uuid, person, dailyExposure: photo.daily_exposure ?? null }));
//         setTimeout(() => {
//           if (direction === "EXIT") { setScreen("idle"); return; }
...
//           const limitMin = photo.daily_exposure?.limit_minutes ?? sysConfig.dailyLimitMin;
//           if (totalMin >= limitMin) setScreen("time_alert");
//           else { resetEpiScan(true); cameraHook.startStream("body1"); if (cameraHook.getAssignment("body2")) cameraHook.startStream("body2"); setScreen("epi_scan"); }
//         }, 900);
//       } else {
//         setFaceStep("error"); setFaceStatusMsg("Rosto não reconhecido"); setFaceSubMsg("Usuário não encontrado — solicite acesso manual"); setFaceProgress(0);
//         setSession((prev) => ({ ...prev, person: null, deniedReason: "user_not_found" }));
//       }
//     } catch (e) { const err = e as { response?: { data?: { detail?: string } }; message?: string }; setFaceStep("error"); setFaceStatusMsg("Erro ao processar"); setFaceSubMsg(err.response?.data?.detail || err.message || ""); setFaceProgress(0); }
//     // eslint-disable-next-line react-hooks/exhaustive-deps
//   }, [faceStep, cameraHook, direction, sysConfig, resetEpiScan]);
...
//   const handleCaptureEpi = useCallback(async () => {
//     if (epiStep !== "ready") return; epiAutoCapture.current = false;
//     const personCode = sessionRef.current.person?.personCode?.trim();
//     const personName = sessionRef.current.person?.personName?.trim();
//     if (!personCode || !personName) { setEpiStep("error"); setEpiStatusMsg("❌ Usuário não encontrado — Acesso negado"); setDoorStatus("closed"); setSession((prev) => ({ ...prev, missingEpi: [], deniedReason: "user_not_found", epiResult: { compliant: false } })); setTimeout(() => setScreen("access_denied"), 1500); return; }
//     const hasBody2 = !sysConfig.useSingleCamera && !!cameraHook.getAssignment("body2");
//     try {
//       setEpiStep("capturing"); setEpiStatusMsg("Capturando frames…");
//       const captureRole: CamRole = sysConfig.useSingleCamera ? "face" : "body1";
//       const { blob: blob1, dataUrl: url1 } = await cameraHook.captureFrame(captureRole); setEpiCaptureUrl1(url1);
//       let blob2: Blob | null = null;
//       if (hasBody2) { try { const f2 = await cameraHook.captureFrame("body2"); setEpiCaptureUrl2(f2.dataUrl); blob2 = f2.blob; } catch (e2) { console.warn("[EPI] Body2 capture failed:", (e2 as Error).message); } }
//       setEpiStep("processing"); setEpiStatusMsg("Detectando EPIs… aguarde");
//       const epiSessionData = await api.startValidationSession({ direction, door_id: sysConfig.doorId, zone_id: sysConfig.zoneId });
//       const epiSessionUuid = epiSessionData.session_uuid || epiSessionData.sessionUuid!;
//       validationStream.open(epiSessionUuid);
//       const photo1 = await api.sendValidationPhoto(epiSessionUuid, blob1, { photoType: "body", cameraId: 2 });
//       let finalResult: EpiResult = { ...photo1, compliant: resolveCompliant(photo1) };
//       if (blob2) { try { const photo2 = await api.sendValidationPhoto(epiSessionUuid, blob2, { photoType: "body", cameraId: 3 }); const photo2Compliant = resolveCompliant(photo2); if (!photo2Compliant) finalResult = { ...photo2, compliant: photo2Compliant }; } catch (e3) { console.warn("[EPI] Body2 send failed:", (e3 as Error).message); } }
//       try { await api.closeValidationSession(epiSessionUuid, sessionRef.current.person?.personCode); } catch (_) {}
//       setEpiResult(finalResult); setEpiStep("done");
//       setSession((prev) => ({ ...prev, epiResult: finalResult, sessionUuid: epiSessionUuid, deniedReason: null }));
//       if (finalResult.compliant) { setEpiStatusMsg("✅ EPI Completo — Acesso liberado"); setTimeout(() => setScreen("access_granted"), 1200); }
//       else { setEpiStatusMsg("❌ EPI Incompleto — Acesso negado"); const missing = (finalResult.missing || finalResult.missing_ppe || []).map(epiLabel); setSession((prev) => ({ ...prev, missingEpi: missing, deniedReason: "epi_incomplete" })); setDoorStatus("closed"); if (lockState.connected) lockState.sendLed("red"); setTimeout(() => setScreen("access_denied"), 1200); }
//     } catch (e) { setEpiStep("error"); const errorMsg = (e as any)?.response?.data?.detail || (e as Error)?.message || "Erro desconhecido"; setEpiStatusMsg(`Erro na detecção de EPI: ${errorMsg}`); }
//     // eslint-disable-next-line react-hooks/exhaustive-deps
//   }, [epiStep, cameraHook, direction, sysConfig, lockState.connected, lockState.sendLed, validationStream]);

//   const handleRetryEpi = useCallback(() => resetEpiScan(false), [resetEpiScan]);

//   // ─── ACTION: Decisão do WS EPI Stream ──────────────────────────────────────
//   // Chamado pelo EpiScanScreen quando o WebSocket stream toma uma decisão.
//   // Não faz chamadas de rede — apenas atualiza o estado e navega para a tela certa.
//   const handleEpiStreamDecision = useCallback((decision: EpiStreamDecision) => {
//     console.log(`[EpiStream] Decisão: ${decision.access_decision} epi=${(decision.compliance_rate * 100).toFixed(0)}% face=${(decision.face_rate * 100).toFixed(0)}%`);

//     const personCode = decision.person_code ?? sessionRef.current.person?.personCode ?? null;
//     const personName = decision.person_name ?? sessionRef.current.person?.personName ?? null;

//     if (decision.access_decision === "GRANTED") {
//       setSession((prev) => ({
//         ...prev,
//         epiResult: { compliant: true, missing: [] },
//         deniedReason: null,
//         person: personCode && personName
//           ? { personCode, personName, confidence: decision.face_rate }
//           : prev.person,
//       }));
//       setScreen("access_granted");

//     } else if (decision.access_decision === "DENIED_EPI") {
//       setSession((prev) => ({ ...prev, epiResult: { compliant: false }, missingEpi: [], deniedReason: "epi_incomplete" }));
//       if (lockState.connected) lockState.sendLed("red");
//       setScreen("access_denied");

//     } else {
//       // DENIED_FACE
//       setSession((prev) => ({ ...prev, epiResult: { compliant: false }, missingEpi: [], deniedReason: "user_not_found" }));
//       if (lockState.connected) lockState.sendLed("red");
//       setScreen("access_denied");
//     }
//   }, [lockState.connected, lockState.sendLed]);

//   const handleSaveConfigModal = useCallback(async () => {
//     try { setSaving(true); handleSaveConfig(localConfig); if (epiConfig?.required_ppe) await api.saveEpiConfig({ required_ppe: epiConfig.required_ppe }); setSaved(true); setTimeout(() => setSaved(false), 2000); }
//     catch (e) { console.error("[useCamAutomation] saveConfig failed:", e); } finally { setSaving(false); }
//   }, [localConfig, epiConfig, handleSaveConfig]);
...
//     cameraHook, lockState, validationStream,
//     idleState: { dashboard, loadingDash, refreshDashboard },
//     faceScanState: { step: faceStep, captureUrl: faceCaptureUrl, progress: faceProgress, statusMsg: faceStatusMsg, subMsg: faceSubMsg, countdown: faceCountdown },
//     epiScanState: { step: epiStep, captureUrl1: epiCaptureUrl1, captureUrl2: epiCaptureUrl2, statusMsg: epiStatusMsg, countdown: epiCountdown, result: epiResult },
//     configState: { localConfig, epiConfig, saving, saved, setLocalConfig, setEpiConfig, handleSave: handleSaveConfigModal },
//     permanenceState: { people, loading: loadingPeople, fetchPeople },
//     handleStartEntry, handleStartExit, handleGoIdle,
//     handleTimeOverride, handleRetryFromDenied, handleSaveConfig,
//     handleCaptureFace, handleRetryFace, handleCaptureEpi, handleRetryEpi,
...
export function useCamAutomation(): UseCamAutomationReturn {
  ...
  const handleStartExit = useCallback(() => { setDirection("EXIT"); resetSession(); resetFaceScan(true); cameraHook.startStream("face"); setScreen("face_scan"); }, [resetSession, resetFaceScan, cameraHook]);
  const handleGoIdle = useCallback(() => { cameraHook.stopAllStreams(); resetSession(); resetFaceScan(false); resetEpiScan(false); validationStream.close(); setDoorStatus("closed"); setScreen("idle"); }, [cameraHook, resetSession, resetFaceScan, resetEpiScan, validationStream]);
  const handleTimeOverride = useCallback(() => { resetEpiScan(false); if (!sysConfig.useSingleCamera) { cameraHook.startStream("body1"); if (cameraHook.getAssignment("body2")) cameraHook.startStream("body2"); } setScreen("epi_scan"); }, [cameraHook, resetEpiScan, sysConfig.useSingleCamera]);
  const handleRetryFromDenied = useCallback(() => { setSession((prev) => ({ ...prev, epiResult: null, missingEpi: [], deniedReason: null })); resetEpiScan(false); setScreen("epi_scan"); }, [resetEpiScan]);
  const handleSaveConfig = useCallback((newConfig: Partial<SysConfig>) => { setSysConfig((prev) => ({ ...prev, ...newConfig })); }, []);
    ...
    try {
      ...
      if (resolvedCode && resolvedName) {
        ...
        setTimeout(() => {
          ...
          const limitMin = photo.daily_exposure?.limit_minutes ?? sysConfig.dailyLimitMin;
          if (totalMin >= limitMin) setScreen("time_alert");
          else { resetEpiScan(false); cameraHook.startStream("body1"); if (cameraHook.getAssignment("body2")) cameraHook.startStream("body2"); setScreen("epi_scan"); }
        }, 900);
      } else {
        setFaceStep("error"); setFaceStatusMsg("Rosto não reconhecido"); setFaceSubMsg("Usuário não encontrado — solicite acesso manual"); setFaceProgress(0);
        setSession((prev) => ({ ...prev, person: null, deniedReason: "user_not_found" }));
