...
//   // Documentação: POST http://<ip>/unlock  body: { duration_ms: number }
...
//   cameraSourceType: Record<CamRole, CameraSourceType>;
//   cameraIpUrl: Record<CamRole, string | null>;
//   // ─── Configuração da fechadura ESP32 (MIXHUB IoT) ─────────────────────────
//   // IP local do ESP32 na rede. Ex: "192.168.68.100"
//   lockIpAddress: string;
//   // Tempo em ms que a fechadura permanece destrancada após o unlock
...
//       (localStorage.getItem(LS_KEYS_TYPE.face) as CameraSourceType) || "local",
//     body1:
//       (localStorage.getItem(LS_KEYS_TYPE.body1) as CameraSourceType) ||
//       "local",
//     body2:
//       (localStorage.getItem(LS_KEYS_TYPE.body2) as CameraSourceType) ||
//       "local",
//   });
...
//     (role: CamRole, deviceId: string | null) => {
//       setAssignments((prev) => ({ ...prev, [role]: deviceId }));
//       localStorage.setItem(LS_KEYS[role], deviceId || "");
//     },
//     [],
...
//     (role: CamRole, type: CameraSourceType) => {
//       setSourceTypes((prev) => ({ ...prev, [role]: type }));
//       localStorage.setItem(LS_KEYS_TYPE[role], type);
//     },
//     [],
...
//         // ACESSO LIBERADO
//         // Fluxo:
//         //   1. Log de auditoria no backend Python (fire-and-forget)
//         //   2. Chama o ESP32 diretamente via POST /unlock (MIXHUB IoT API v2.2.0)
//         //      body: { duration_ms: number }
//         //      resposta esperada: { ok: true, state: "unlocked", duration_ms: 5000 }
//         // ─────────────────────────────────────────────────────────────────────
//         setEpiStatusMsg("✅ EPI Completo — Abrindo fechadura…");

//         // 1. Auditoria no backend Python — não bloqueia o unlock físico
//         api
//           .openDoor({
//             personCode: sessionRef.current.person?.personCode,
//             personName: sessionRef.current.person?.personName,
...
//           })
//           .catch((e) =>
//             console.warn("[EPI] Backend log falhou (não crítico):", e),
//           );

//         // 2. Aciona a fechadura física ESP32
//         try {
//           await api.unlockDoor(
//             sysConfig.lockIpAddress,
//             sysConfig.lockDurationMs,
//           );
//           setDoorStatus("open");
//           setEpiStatusMsg("✅ EPI Completo — Acesso liberado");
//         } catch (lockErr) {
//           // Validação foi aprovada mas a fechadura não respondeu
//           // Marca como alert para exibir aviso no TopBar sem bloquear a tela de sucesso
//           console.error("[EPI] Falha ao acionar fechadura ESP32:", lockErr);
//           setDoorStatus("alert");
//           setEpiStatusMsg("⚠️ Validado, mas falha na fechadura — Chame o responsável");
//         }
