...
const getApiBaseUrl = (): string => {
  ...
  if (savedEndpoint) {
    return `${savedEndpoint}/dashboard/orders`;
  }
  
  // Fallback para API padrão (pode ser qualquer uma das duas)
  return 'https://apinode.smartxhub.cloud/api/dashboard/orders';
};
...
const fetchAPI = async <T>(
  ...
  const API_BASE_URL = getApiBaseUrl(); // 👈 Pega URL dinâmica
  const queryString = params ? buildQueryString(params) : "";
  const url = `${API_BASE_URL}${endpoint}${queryString ? `?${queryString}` : ""}`;

  const response = await fetch(url, {
    headers: {
      'Authorization': `Bearer ${sessionStorage.getItem('token')}` // 👈 Adiciona token
    }
  });
