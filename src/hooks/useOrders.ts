...
export interface OrdersSummary {
  ...
  items_commissioned: number;
  items_in_transfer: number;
  items_transferred: number;
  items_in_progress: number;
  items_out_for_delivery: number;
...
export const useOrders = () => {
  ...
  const [jobTypePerformance, setJobTypePerformance] = useState<JobTypePerformance[]>([]);
  ...
  const [itemsPagination, setItemsPagination] = useState<PaginationInfo | null>(null);
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      const data = await fetchAPI<DashboardData>(`/${companyId}/dashboard-data`);
  ...
  const fetchKPISummary = useCallback(async () => {
    ...
    try {
      const data = await fetchAPI<OrdersKPISummary>(`/${companyId}/kpi-summary`);
      setKpiSummary(data);
      return data;
    } catch (err) {
  ...
  const fetchJobTypePerformance = useCallback(async () => {
    ...
    try {
      const data = await fetchAPI<JobTypePerformance[]>(`/${companyId}/job-type-performance`);
      setJobTypePerformance(data);
      return data;
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Failed to fetch job type performance';
      setError(errorMessage);
  ...
  const fetchTimeline = useCallback(async (days: number = 30) => {
    ...
    try {
      const data = await fetchAPI<TimelineData[]>(`/${companyId}/timeline`, { days });
      setTimeline(data);
      return data;
    } catch (err) {
  ...
  const fetchItemStatusDistribution = useCallback(async () => {
    ...
    try {
      const data = await fetchAPI<ItemStatusDistribution[]>(`/${companyId}/item-status-distribution`);
      setItemStatusDistribution(data);
      return data;
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Failed to fetch item status distribution';
      setError(errorMessage);
  ...
  const fetchOrders = useCallback(async (filters?: OrderFilters, limit: number = 100) => {
    setLoading(true);
    setError(null);
    ...
    try {
      ...
      const data = await fetchAPI<OrdersSummary[]>(`/${companyId}/orders-summary`, params);
      setOrders(data);
      return data;
    } catch (err) {
