...
// Hook HÍBRIDO: MediaPipe local para overlay + validação de corpo completo
// Não usa backend API - processa tudo localmente com MediaPipe Pose
...
export function useBodyFramingDetection({
  ...
  // ✅ Estado para armazenar resultados do MediaPipe (para overlay)
  ...
  // Analisar resultado do MediaPipe
  const analyzeFraming = useCallback((results: Results) => {
    // ✅ Armazenar resultados para desenhar overlay
  ...
  return {
    ...
    shouldBlock: feedback.status !== 'well-framed',
    poseResults, // ✅ Para desenhar overlay
