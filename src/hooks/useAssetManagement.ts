...
interface AssetDetail {
  ...
  location_site_current_id?: number;
  location_site_current_name?: string;
  table_location_area_current_id?: number;
  location_area_current_code?: string;
  location_area_current_description?: string;
  location_zone_current_area_id?: number;
  table_location_zone_current_id?: number;
  location_zone_current_code?: string;
  location_zone_current_description?: string;
    ...
    try {
      ...
      if (filters) {
        ...
        if (filters.sites?.length) {
          queryParams.append('sites', filters.sites.join(','));
        }
        if (filters.areas?.length) {
          queryParams.append('areas', filters.areas.join(','));
        }
        if (filters.zones?.length) {
          queryParams.append('zones', filters.zones.join(','));
        }
      }
